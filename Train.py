...
from CellNet import CellNetBaseline
import warnings
import os
...
class TrainModel:
    def __init__(self, model:Sequential, saveFile:str='best_weights_baseline.h5', *args):
        self.CHECKPOINTPATH = '/content/gdrive/MyDrive/cell tower/predictions/weights'+saveFile
        self.VALDIR = '/content/gdrive/MyDrive/cell tower/data/validation'
        self.TRAINDIR = '/content/gdrive/MyDrive/cell tower/data/train'
        self.IMGWIDTH, self.IMGHEIGHT = 50, 50
        self.CHANNELS = 3
        self.class1 = '/cells'
        self.class2 = '/no_cells'

        self.graphPath = '/content/gdrive/MyDrive/cell tower/predictions/history/acc_loss_history.png'
    ...
    def __trackModel__(self)->None:
        ...
        # through the epochs
        epochs = range(len(self.modelHistory.history['accuracy']))
        _, ax1 = plt.subplots()
        ...
        ax2 = ax1.twinx()
        ax2.plot(epochs, self.modelHistory.history['accuracy'], label='train_accuracy', color='green')
        ax2.plot(epochs, self.modelHistory.history['val_accuracy'], label='val_accuracy', color='orange')
...
if __name__ == "__main__":
    ...
    model = CellNetBaseline.build(width, height, channels)
